...
function aplicar_photon_en_biblioteca_medios($url) {
    ...
    if (is_admin()) {
        ...
        if ($current_screen && $current_screen->base === 'upload') {
            // Aplica Photon a la URL de la imagen
            $url = aplicar_photon($url);
        }
    }
    return $url; // Devuelve la URL modificada si se está en la biblioteca de medios, de lo contrario, devuelve la URL original
}
...
function aplicar_photon_en_biblioteca_medios_js($response, $attachment, $meta) {
    if (is_admin()) {
        ...
        $response['sizes']['full']['url'] = aplicar_photon($image_url); // Asegura que la URL completa también esté formateada
    }
    return $response;
}
...
function aplicar_photon($url){
    ...
    return $url; // Devuelve la URL original si ya está usando Photon
}
